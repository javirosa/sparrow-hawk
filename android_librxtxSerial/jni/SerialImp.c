...
/*----------------------------------------------------------
 to_native_speed
   ...
   comments:   Only the lowest level code should know about
               the magic constants.
----------------------------------------------------------*/
int to_native_speed( JNIEnv *env, jint speed )
{
	LEAVE( "RXTXPort:to_native_speed" );
	switch( speed ) {
		case 0:		return B0;
		case 50:	return B50;
		case 75:	return B75;
...
/* I don't think this is universal.. older UARTs never did these.  taj */
	...
	{
		LEAVE( "RXTXPort:to_native_speed: Error condition" );
		return -1;
	}
}
...
/*----------------------------------------------------------
 from_native_speed
   ...
   accept:     speed_t constant
   perform:    convert a speed_t constant to bits-per-second
   ...
   comments:   Only the lowest level code should know about
               the magic constants.
----------------------------------------------------------*/
int from_native_speed( JNIEnv *env, jint speed )
{
	LEAVE( "RXTXPort:from_native_speed" );
	switch( speed ) {
		case B0:	return 0;
		case B50:	return 50;
		case B75:	return 75;
...
/* I don't think this is universal.. older UARTs never did these.  taj */
	...
	{
		LEAVE( "RXTXPort:from_native_speed: Error condition" );
		return -1;
	}
}
...
/* return; OH CRAP */
	...
	sprintf( message, "::::RXTXPort:Write %d %d %d -> %d \n", 
		         fd, total, offset, count  );
	report_verbose( message );
		if(result >0){
			total += result;
		}
...
changeIspeed
   ...
   accept:     ispeed, bits-per-second
   perform:    change ispeed
   exceptions: IOExcpetion if failed
   comments:
----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(changeIspeed)( JNIEnv *env,
	jobject jobj, jint speed )
{
	int fd = get_java_var( env, jobj,"fd","I" );
	report_time_start( );
	ENTER( "RXTXPort:changeIspeed()" );
	...
	if( tcgetattr( fd, &ttyset ) < 0 )
	{
		report( "changeIspeed: Cannot Get Serial Port Settings\n" );
		goto fail;
	}
	...
	if( cfsetispeed( &ttyset, cspeed ) < 0 )
	{
		report( "changeIspeed: Cannot Change Ispeed!\n" );
		goto fail;
	}

	report_time_end( );
	LEAVE( "RXTXPort:changeIspeed()" );
	return( JNI_TRUE );
...
fail:
	...
	throw_java_exception( env, IO_EXCEPTION, "nativechangeIspeed",
		strerror( errno ) );
	return( JNI_FALSE );
...
changeOspeed
   ...
   accept:     ospeed, bits-per-second
   perform:    change ospeed
   exceptions: IOExcpetion if failed
   comments:
----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL RXTXPort(changeOspeed)( JNIEnv *env,
	jobject jobj, jint speed )
{
	int fd = get_java_var( env, jobj,"fd","I" );
	report_time_start( );
	ENTER( "RXTXPort:changeOspeed()" );
	...
	if( tcgetattr( fd, &ttyset ) < 0 )
	{
		report( "changeOspeed: Cannot Get Serial Port Settings\n" );
		goto fail;
	}
	...
	{
		report( "changeOspeed: Cannot Change Ospeed!\n" );
		goto fail;
	}

	report_time_end( );
	LEAVE( "RXTXPort:changeOspeed()" );
	return( JNI_TRUE );
...
fail:
	...
	throw_java_exception( env, IO_EXCEPTION, "nativeChangeOspeed",
		strerror( errno ) );
	return( JNI_FALSE );
...
*/
    	...
    	baudrate = from_native_speed( cfgetispeed(&ttyset) );
#endif
...
/*----------------------------------------------------------
RXTXPort.nativeStaticSetBaudRate

   accept:      filename
   perform:     set the baud rate (not all buads are handled yet)
   return:      return the result
   exceptions:
   comments:    simple test for preopened ports
----------------------------------------------------------*/
JNIEXPORT jboolean JNICALL Java_gnu_io_RXTXPort_nativeStaticSetBaudRate
	(JNIEnv *env, jobject jobj, jstring jstr, jint baudrate)
{
	const char *filename = (*env)->GetStringUTFChars( env, jstr, 0 );
	int fd = find_preopened_ports( filename );
	struct termios ttyset;
	...
	{
		/* Exception? FIXME */
		report( "nativeStaticSetBaudRate: Cannot Get Serial Port FD\n" );
		goto fail;
	}
	if( tcgetattr( fd, &ttyset ) < 0 )
	{
		report( "nativeStaticSetBaudRate: Cannot Get Serial Port Settings\n" );
		goto fail;
	}
	...
	if( cfsetispeed(&ttyset, cspeed) < 0)
	{
		report( "nativeStaticSetBaudRate: Cannot Set Serial Port ispeed\n" );
		goto fail;
	}

	if( cfsetospeed(&ttyset, cspeed) < 0)
	{
		report( "nativeStaticSetBaudRate: Cannot Set Serial Port ospeed\n" );
		goto fail;
	}
...
fail:
	LEAVE( "RXTXPort:nativeStaticSetBaudRate()" );
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *java_vm, void *reserved)
{
	...
	report_verbose("Testing the version.\n");
